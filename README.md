...
> 重要变更：后端已从 Node.js + Express 全量迁移到 **Python/FastAPI**，并保持 API 兼容。当前 Docker 镜像只运行 FastAPI 后端，**不再在容器内部集成 Nginx**，推荐由宿主机上的 Nginx 做反向代理与静态资源托管。
...
- 前端：Vue 3 + TypeScript + Vite（目录 `melon-tech-web/`）
  - 生产构建产物：`dist/`（在 Docker 构建阶段生成到 `/app/web/dist`）
- 部署建议：
  - 使用 Docker 运行 **后端容器**
  - 使用宿主机 Nginx（或其它反向代理）：
    - 反向代理 `/api` 到后端容器端口
    - 托管前端静态资源（来自本地构建或镜像中的 `/app/web/dist`）
...
> 当前 Docker 镜像仅包含 **Python/FastAPI 后端**。前端静态资源会在构建阶段生成到 `/app/web/dist`，应由 **容器外的 Nginx** 或其它 Web 服务器托管。
...
docker run -d \
  --name msut-auth-app \
  -p 1122:3400 \        # 本地 1122 -> 容器 3400（便于调试）
  -p 3400:3400 \        # 直接访问后端 API
...
- 容器内部固定监听 `PORT=3400`（可通过环境变量覆盖）。
- SQLite 数据库存放在容器内 `/app/server/data/data.sqlite`，建议挂载卷 `msut-data`。
...
2. 在服务器上使用 `docker-compose up -d` 启动后端容器。

3. 在宿主机 Nginx 中配置：

   - 静态资源根目录指向前端构建产物（例如从仓库单独 build 到 `/var/www/msut-web/dist`，或从镜像中 `docker cp` `/app/web/dist` 到宿主机目录）。
   - 将 `/api` 反向代理到后端容器（例如 `http://msut-auth-app:3400` 或 `http://127.0.0.1:3400`）。
...
- Docker 镜像入口脚本会：
  ...
  - 使用 `gosu` 将 FastAPI 进程降权到 `appuser` 用户运行
